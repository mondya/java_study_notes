...
- `CountDownLatch`用于等待一个或多个线程完成某个操作，然后再继续执行其他操作。
- 它通过一个计数器来实现，计数器的初始值可以设定为一个正整数，表示需要等待的线程数。
- 每个线程在完成任务后会调用`countDown()`方法，该方法将计数器的值减一。
- 其他等待的线程可以通过调用`await()`方法来等待计数器的值变为零，一旦计数器的值为零，所有等待的线程会被释放，继续执行后续操作。

`Semaphore`:

- `Semaphore`用于控制同时访问某个资源的线程数量。
- 它通过一个计数器和一组获取和释放资源的方法来实现。
- 计数器的初始值表示可用的许可证数量，每个线程在访问资源之前需要获取一个许可证。
- 当许可证数量为零时，线程需要等待其他线程释放许可证；而当许可证数量不为零时，线程可以获得许可证并继续执行任务。
- 线程在完成对资源的访问后，需要释放许可证，以便其他线程可以获取许可证并访问资源。
