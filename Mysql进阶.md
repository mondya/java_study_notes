...
在对某个表执行SELECT、INSERT、DELETE、UPDATE操作时，InnoDB存储引擎是不会为这个表添加表级别的S锁或者X锁的。在对某个表执行一些诸如ALTER TABLE, DROP TABLE这类DDL语句时，其他事务对这个表并发执行例如SELECT,INSERT,DELETE,UPDATE的语句会发生阻塞。同理，某个事务对表执行SELECT,INSERT,DELETE,UPDATE语句时，在其他会话中执行DDL语句也会发生阻塞
