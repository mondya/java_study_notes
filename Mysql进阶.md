...
在对某个表执行SELECT、INSERT、DELETE、UPDATE操作时，InnoDB存储引擎是不会为这个表添加表级别的S锁或者X锁的。在对某个表执行一些诸如ALTER TABLE, DROP TABLE这类DDL语句时，其他事务对这个表并发执行例如SELECT,INSERT,DELETE,UPDATE的语句会发生阻塞。同理，某个事务对表执行SELECT,INSERT,DELETE,UPDATE语句时，在其他会话中执行DDL语句也会发生阻塞。这个过程是通过元数据锁结构实现的。

```mysql
lock tables 表名 read(S：读锁)/ write（X：写锁）;
```
...
select * from user; // 自己可读
update user set name = 'admin' where id = 1; // 自己不可写
select * from student; // 自己不可操作其他表
unlock tables;
```
...
表明某个事务正在某些行持有了锁或者该事务准备去掉持有锁

![image-20230815220004637](https://gitee.com/cnuto/images/raw/master/image/image-20230815220004637.png)
...
```mysql
begin ;
update user set name = 'admin' where id = 24; // 如果此时有相同操作在更改id=24的数据，会阻塞；不是id=24，能够正常执行，说明意向排他锁和意向排他锁之间兼容
commit ;
```
...
AUTO-INC锁是使用含有AUTO_INCREMENT列的表中插入数据时需要获取的一种特殊的表级锁，在执行插入语句时就在表级别加一个AUTO-INC锁，然后为每条待插入的的AUTO_INCREMENT修饰的列分配递增的值，在该语句执行结束后，在把AUTO-INC锁释放。==一个事务在持有AUTO-INC锁的过程中，其他事务的插入语句都要被阻塞==，可以保证一个语句中分配的递增值是连续的。

##### 元数据锁

当对一个表做增删改查操作时，加MDL读锁；当要对表做结构变更操作时，加MDL写锁。

![image-20230815225208792](https://gitee.com/cnuto/images/raw/master/image/image-20230815225208792.png)

#### 行锁（Row Lock）

优点：锁定粒度小，发生锁冲突概率低，并发度高。
...
![image-20230816212626145](https://gitee.com/cnuto/images/raw/master/image/image-20230816212626145.png)

图中id=8的记录加了gap锁，表明==不允许别的事务在id=8的记录前边的间隙插入新记录==，既id列值(3,8)区间的记录不允许立即插入。==gap锁的提出仅仅是为了防止插入幻影记录而提出的==
...
|                                                           | insert into student values(5, 'xxx'); 阻塞                |
| insert into student values(5, 'xxx');                     |                                                           |

session1和session2进入互相等待对方资源的释放，进入了死锁状态

- 直接进入等待，直到超时。通过`innodb_lock_wait_timeout`参数配置
- 发起死锁检测，主动回滚死锁链条中的某个事务（将持有最少行级锁排他锁的事务进行回滚），让其他事务得以继续始终。`innodb_deadlock_detect`设置为on表示开启该逻辑

##### 临键锁（Next-Key Locks）

![image-20230816220641041](https://gitee.com/cnuto/images/raw/master/image/image-20230816220641041.png)

##### 插入意向锁（Insert Intention Locks）

![image-20230816220514854](https://gitee.com/cnuto/images/raw/master/image/image-20230816220514854.png)

#### 页锁

页锁就是在页的粒度上进行锁定，锁定的数据资源比行锁要多，因为一个页中可以有多个行记录，数据浪费，==页锁的开销介于表锁和行锁之间，会出现死锁。锁定粒度介于表锁和行锁之间，并发度一般==。每个成绩的锁数量是有限制的，因为锁会占用内存空间，锁空间的大小是有限的。当某个成绩的锁数量超过了这个层级的阈值，就会进行锁升级。锁升级就是用更大粒度的锁替代多个更小粒度的锁。

### 从对待锁的态度划分：乐观锁，悲观锁

#### 悲观锁

悲观锁总是假设最坏情况，拿数据时都认为数据会被修改，所以每次查询数据都会上锁，其他查询操作会被阻塞直到拿到锁（==共享资源每次只给一个线程使用，其他线程阻塞，用完后在把资源转让给其他线程==）

#### 乐观锁

乐观锁认为对同一个数据的并发操作不会总是发生，属于小概率事件，不用每次都对数据上锁，但是在更新时会判断此时有没有别的线程在更新此数，也就是==不采用数据自身的锁机制，而是通过程序来实现==
...
不论是表锁或者行锁，都是在事务执行过程中生成，哪个事务生成了这个锁结构，这里就记录了这个事务的信息。
...
- 表锁/行锁信息：
  - 表锁：记载对哪个表加的锁，以及一些其他信息
  - 行锁：SpaceId:记录所在表空间；Page Number：记录所在页号。n_bits：对于行锁来说，一条记录对应一个比特位，一个页面中包含很多记录，用不同的比特位来区分到底是哪一条记录加了锁。

## MVCC

### ReadView

在MVCC机制中，多个事务对同一行记录进行更新会产生多个历史快照，这些历史快照保存在undo log里。如果一个事务想要查询这个行记录，需要读取哪个版本的记录？这时就需要用到ReadView了，它帮我们解决了行的可见性问题。ReadView是事务在使用MVCC机制进行快照读操作时产生的读视图。当事务启动时，会生成数据库系统当前的一个快照，InnoDB为每个事务构造了一个数组，用来记录并维护系统当前活跃事务的ID。

- `creator_trx_id`：创建这个ReadView的事务id（只有在对表中的数据执行INSERT, DELETE, UPDATE这些操作时才会为事务分配事务id,否则在一个只读事务中的事务id都是默认为0）
- `trx_ids`：表示在生成ReadView时当前系统中活跃的读写事务的事务id列表
- `up_limit_id`：活跃的事务中最小的事务Id.
- `low_limit_id`：表示生成的ReadView时系统中应该分配给下一个事务的id值，low_limit_id是系统最大的事务id，注意是系统中的事务，需要区别于正在活跃的事务Id。（low_limit_id并不是trx_ids中的最大值，事务id是递增分配的。比如，现在有id为1，2，3这三个事务，之后id=3的事务提交了，那么一个新的事务在生成readView时，trx_ids就包括1和2，up_limit_id的值就是1，low_limit_id的值就是4）

规则：

- 如果被访问的trx_id属性值和ReadView中的creator_trx_id相同，意味着当前事务在访问自身修改过的记录，可见
- 如果被访问的trx_id值小于ReadView中的up_limit_trx值，表明生成该版本的事务在当前事务生成ReadView前已经提交，可见
- 如果被访问的trx_id值大于或等于low_limit_trx值，表明生成该版本的事务在当前事务生成ReadView后才开启，不可见
- 如果被访问的trx_id介于low_limit_trx和up_limit_trx之间，那么就需要判断是否在trx_ids列表中
  - 如果在，说明创建ReadView时生成该版本的事务还是活跃的，该版本不可以被访问
  - 如果不在，说明事务已经被提交，该版本可以被访问
